...
public class AddExaminationUnitSteps extends UnitSteps {
    ...
    @Given("Martha has chosen to add a new examination for {word}")
    public void martha_has_chosen_to_add_a_new_examination_for_tom(String name) {
    ...
    public void the_examination_should_be_added_to_tom_s_examinations() {
        Examination examination = context.getExamination();
        Patient patient = context.getPatient();
        patient.getMostRecentExamination();
        assertEquals(examination, patient.getMostRecentExamination());
    ...
    public void martha_registers_gr_as_weight(Integer width) {
        Examination examination = context.getExamination();
        examination.setWeight(width);
        Patient patient = context.getPatient();
        patient.addExamination(examination);
        ///FOR ERROR
        List<String> errors = validatePatient(patient);
